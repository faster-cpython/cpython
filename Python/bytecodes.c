...
// BEGIN BYTECODES //
        ...
        pure inst(END_SEND, (receiver, value -- val)) {
            ...
            val = value;
            DEAD(value);
            PyStackRef_CLOSE(tstate, receiver);
        }
       ...
       op(_GUARD_BINARY_OP_EXTEND, (descr/4, left, right -- left, right)) {
        ...
        op(_BINARY_SLICE, (container, start, stop -- res)) {
            ...
            PyStackRef_CLOSE(tstate, container);
            ERROR_IF(res_o == NULL, error);
            res = PyStackRef_FromPyObjectSteal(res_o);
        }
        ...
        op(_BINARY_OP_SUBSCR_STR_INT, (str_st, sub_st -- res)) {
            ...
            DEAD(sub_st);
            PyStackRef_CLOSE(tstate, str_st);
        ...
        op(_STORE_SUBSCR_LIST_INT, (value, list_st, sub_st -- )) {
            ...
            PyStackRef_CLOSE_SPECIALIZED(sub_st, _PyLong_ExactDealloc);
            DEAD(sub_st);
            PyStackRef_CLOSE(tstate, list_st);
            Py_DECREF(old_value);
        }
        ...
        inst(STORE_SUBSCR_DICT, (unused/1, value, dict_st, sub -- )) {
            ...
            int err = _PyDict_SetItem_Take2((PyDictObject *)dict,
                                            ...
                                            PyStackRef_AsPyObjectSteal(value));
            PyStackRef_CLOSE(tstate, dict_st);
            ERROR_IF(err, error);
        }
        ...
        inst(GET_AWAITABLE, (iterable -- iter)) {
            ...
            PyStackRef_CLOSE(tstate, iterable);
            ERROR_IF(iter_o == NULL, error);
            iter = PyStackRef_FromPyObjectSteal(iter_o);
        }
        ...
        inst(LOAD_FROM_DICT_OR_GLOBALS, (mod_or_class_dict -- v)) {
            ...
            PyStackRef_CLOSE(tstate, mod_or_class_dict);
            ERROR_IF(err < 0, error);
            if (v_o == NULL) {
        ...
        inst(LOAD_FROM_DICT_OR_DEREF, (class_dict_st -- value)) {
            ...
            PyStackRef_CLOSE(tstate, class_dict_st);
        ...
        inst(LIST_EXTEND, (list_st, unused[oparg-1], iterable_st -- list_st, unused[oparg-1])) {
            ...
            if (none_val == NULL) {
                ...
                PyStackRef_CLOSE(tstate, iterable_st);
                ERROR_IF(true, error);
            }
        ...
        inst(DICT_UPDATE, (dict, unused[oparg - 1], update -- dict, unused[oparg - 1])) {
            ...
            if (err < 0) {
                ...
                PyStackRef_CLOSE(tstate, update);
                ERROR_IF(true, error);
            }
            PyStackRef_CLOSE(tstate, update);
        }
        ...
        inst(LOAD_SUPER_ATTR_METHOD, (unused/1, global_super_st, class_st, self_st -- attr, self_or_null)) {
            ...
            if (method_found) {
                ...
                PyStackRef_CLOSE(tstate, self_st);
...
// invalid by the time we actually try to fetch the item.
        ...
        op(_CALL_BUILTIN_O, (callable[1], self_or_null[1], args[oparg] -- res)) {
            ...
            DEAD(args);
            DEAD(self_or_null);
            PyStackRef_CLOSE(tstate, callable[0]);
            ERROR_IF(res_o == NULL, error);
            res = PyStackRef_FromPyObjectSteal(res_o);
        }
        ...
        inst(CALL_LEN, (unused/1, unused/2, callable[1], self_or_null[1], args[oparg] -- res)) {
            ...
            PyStackRef_CLOSE(tstate, arg_stackref);
            DEAD(args);
            DEAD(self_or_null);
            PyStackRef_CLOSE(tstate, callable[0]);
            res = PyStackRef_FromPyObjectSteal(res_o);
        ...
        inst(CALL_LIST_APPEND, (unused/1, unused/2, callable, self, arg -- )) {
            ...
            PyStackRef_CLOSE(tstate, self);
            PyStackRef_CLOSE(tstate, callable);
            ERROR_IF(err, error);
        #if TIER_ONE
        ...
        op(_CALL_METHOD_DESCRIPTOR_NOARGS, (callable[1], self_or_null[1], args[oparg] -- res)) {
            ...
            _Py_LeaveRecursiveCallTstate(tstate);
            assert((res_o != NULL) ^ (_PyErr_Occurred(tstate) != NULL));
            PyStackRef_CLOSE(tstate, self_stackref);
            DEAD(args);
            DEAD(self_or_null);
            PyStackRef_CLOSE(tstate, callable[0]);
            ERROR_IF(res_o == NULL, error);
        ...
        op(_DO_CALL_KW, (callable[1], self_or_null[1], args[oparg], kwnames -- res)) {
                ...
                ((PyFunctionObject *)callable_o)->vectorcall == _PyFunction_Vectorcall)
            {
                ...
                DEAD(self_or_null);
                DEAD(callable);
                PyStackRef_CLOSE(tstate, kwnames);
        ...
        op(_EXPAND_METHOD_KW, (callable[1], self_or_null[1], unused[oparg], unused -- callable[1], self_or_null[1], unused[oparg], unused)) {
            ...
            callable[0] = PyStackRef_FromPyObjectNew(((PyMethodObject *)callable_o)->im_func);
            assert(PyStackRef_FunctionCheck(callable[0]));
            PyStackRef_CLOSE(tstate, callable_s);
        }
        ...
        op(_MAKE_CALLARGS_A_TUPLE, (func, unused, callargs, kwargs_in -- func, unused, tuple, kwargs_out)) {
            ...
            if (PyTuple_CheckExact(callargs_o)) {
                ...
                kwargs_out = kwargs_in;
                DEAD(kwargs_in);
                PyStackRef_CLOSE(tstate, callargs);
        ...
        op(_DO_CALL_FUNCTION_EX, (func_st, null, callargs_st, kwargs_st -- result)) {
            ...
            PyStackRef_XCLOSE(tstate, kwargs_st);
            PyStackRef_CLOSE(tstate, callargs_st);
            DEAD(null);
            PyStackRef_CLOSE(tstate, func_st);
        ...
        inst(MAKE_FUNCTION, (codeobj_st -- func)) {
            ...
            PyStackRef_CLOSE(tstate, codeobj_st);
        ...
        tier2 pure op(_POP_TWO_LOAD_CONST_INLINE_BORROW, (ptr/4, pop1, pop2 -- value)) {
            PyStackRef_CLOSE(tstate, pop2);
            PyStackRef_CLOSE(tstate, pop1);
            value = PyStackRef_FromPyObjectImmortal(ptr);
        }
