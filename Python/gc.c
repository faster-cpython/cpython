...
_PyGC_GetObjects(PyInterpreterState *interp, int generation)
{
     ...
     */
    if (result == NULL || generation >= NUM_GENERATIONS) {
        return result;
    }
    if (generation < 0) {
        ...
        for (int i = 0; i < NUM_GENERATIONS; i++) {
            if (append_objects(result, &gcstate->candidates[i].head)) {
                goto error;
            }
        }
    }
...
//     gc_list_set_visited(&gcstate->young.head);
...
_PyGC_Collect(PyThreadState *tstate, int generation, _PyGC_Reason reason)
{
    ...
    for (int i = 0; i < NUM_GENERATIONS; i++) {
        validate_list(&gcstate->candidates[i].head, collecting_clear_unreachable_clear);
    }
