...
peephole_opt(_PyInterpreterFrame *frame, _PyUOpInstruction *buffer, int buffer_size)
{
    ...
    for (int pc = 0; pc < buffer_size; pc++) {
        ...
        switch(opcode) {
            ...
            case _POP_TOP:
            {
                int prev_opcode = buffer[pc-1].opcode;
                switch (prev_opcode) {
                    case _LOAD_FAST:
                    case _LOAD_CONST:
                    case _LOAD_CONST_INLINE:
                    case _LOAD_CONST_INLINE_BORROW:
                        buffer[pc-1].opcode = buffer[pc].opcode = _NOP;
                }
                break;
            }
            case _JUMP_TO_TOP:
...
)
{
    ...
    int mod = _Py_Tier2_Specialize(
        _PyFrame_GetCode(frame), buffer, buffer_size, curr_stackentries, dependencies);
    if (mod) {
        // printf("MODIFIED ");
        // dump_uops(buffer, buffer_size);
    }
